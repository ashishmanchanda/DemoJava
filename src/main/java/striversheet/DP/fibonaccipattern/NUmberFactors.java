package striversheet.DP.fibonaccipattern;
class ExpressNumber {
    public int CountWays(int n) {
        if( n == 0)
            return 1; // base case, we don't need to subtract any thing, so there is on
        if(n == 1)
            return 1; // we can subtract 1 to be left with zero, and that is the only w
        if(n == 2)
